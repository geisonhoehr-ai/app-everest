-- =====================================================
-- MIGRAÇÃO PASSO A PASSO - EXECUTAR NO SUPABASE
-- =====================================================
-- Execute cada bloco separadamente e verifique se dá erro
-- Se der erro, pule para o próximo bloco

-- BLOCO 1: Adicionar coluna TYPE
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns
    WHERE table_schema = 'public' AND table_name = 'quizzes' AND column_name = 'type'
  ) THEN
...
-- BLOCO 2: Adicionar coluna STATUS
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns
...
-- BLOCO 3: Adicionar coluna SCHEDULED_START
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns
    WHERE table_schema = 'public' AND table_name = 'quizzes' AND column_name = 'scheduled_start'
  ) THEN
    ALTER TABLE public.quizzes ADD COLUMN scheduled_start TIMESTAMPTZ;
    RAISE NOTICE 'Coluna scheduled_start adicionada';
  ELSE
    RAISE NOTICE 'Coluna scheduled_start já existe';
  END IF;
END $$;

-- BLOCO 4: Adicionar coluna SCHEDULED_END
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns
    WHERE table_schema = 'public' AND table_name = 'quizzes' AND column_name = 'scheduled_end'
  ) THEN
    ALTER TABLE public.quizzes ADD COLUMN scheduled_end TIMESTAMPTZ;
    RAISE NOTICE 'Coluna scheduled_end adicionada';
  ELSE
    RAISE NOTICE 'Coluna scheduled_end já existe';
  END IF;
END $$;

-- BLOCO 5: Adicionar coluna TOTAL_POINTS
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns
    WHERE table_schema = 'public' AND table_name = 'quizzes' AND column_name = 'total_points'
  ) THEN
    ALTER TABLE public.quizzes ADD COLUMN total_points INTEGER DEFAULT 0;
    RAISE NOTICE 'Coluna total_points adicionada';
  ELSE
    RAISE NOTICE 'Coluna total_points já existe';
  END IF;
END $$;

-- BLOCO 6: Adicionar coluna PASSING_SCORE
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns
    WHERE table_schema = 'public' AND table_name = 'quizzes' AND column_name = 'passing_score'
  ) THEN
    ALTER TABLE public.quizzes ADD COLUMN passing_score INTEGER;
    RAISE NOTICE 'Coluna passing_score adicionada';
  ELSE
    RAISE NOTICE 'Coluna passing_score já existe';
  END IF;
END $$;

-- BLOCO 7: Adicionar coluna SHOW_RESULTS_IMMEDIATELY
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns
    WHERE table_schema = 'public' AND table_name = 'quizzes' AND column_name = 'show_results_immediately'
  ) THEN
    ALTER TABLE public.quizzes ADD COLUMN show_results_immediately BOOLEAN DEFAULT true;
    RAISE NOTICE 'Coluna show_results_immediately adicionada';
  ELSE
    RAISE NOTICE 'Coluna show_results_immediately já existe';
  END IF;
END $$;

-- BLOCO 8: Adicionar coluna SHUFFLE_QUESTIONS
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns
    WHERE table_schema = 'public' AND table_name = 'quizzes' AND column_name = 'shuffle_questions'
  ) THEN
    ALTER TABLE public.quizzes ADD COLUMN shuffle_questions BOOLEAN DEFAULT false;
    RAISE NOTICE 'Coluna shuffle_questions adicionada';
  ELSE
    RAISE NOTICE 'Coluna shuffle_questions já existe';
  END IF;
END $$;

-- BLOCO 9: Adicionar coluna SHUFFLE_OPTIONS
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns
    WHERE table_schema = 'public' AND table_name = 'quizzes' AND column_name = 'shuffle_options'
  ) THEN
    ALTER TABLE public.quizzes ADD COLUMN shuffle_options BOOLEAN DEFAULT false;
    RAISE NOTICE 'Coluna shuffle_options adicionada';
  ELSE
    RAISE NOTICE 'Coluna shuffle_options já existe';
  END IF;
END $$;

-- BLOCO 10: Adicionar coluna ALLOW_REVIEW
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns
    WHERE table_schema = 'public' AND table_name = 'quizzes' AND column_name = 'allow_review'
  ) THEN
    ALTER TABLE public.quizzes ADD COLUMN allow_review BOOLEAN DEFAULT true;
    RAISE NOTICE 'Coluna allow_review adicionada';
  ELSE
    RAISE NOTICE 'Coluna allow_review já existe';
  END IF;
END $$;

-- BLOCO 11: Adicionar coluna INSTRUCTIONS
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns
    WHERE table_schema = 'public' AND table_name = 'quizzes' AND column_name = 'instructions'
  ) THEN
    ALTER TABLE public.quizzes ADD COLUMN instructions TEXT;
    RAISE NOTICE 'Coluna instructions adicionada';
  ELSE
    RAISE NOTICE 'Coluna instructions já existe';
  END IF;
END $$;

-- BLOCO 12: Adicionar coluna CREATED_BY
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns
    ...
    ALTER TABLE public.quizzes ADD COLUMN created_by UUID REFERENCES auth.users(id);
...
-- BLOCO 14: Criar tabela QUIZ_CLASSES
...
-- BLOCO 15: Criar tabela QUIZ_READING_TEXTS
...
-- BLOCO 16: Adicionar colunas em QUIZ_QUESTIONS
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns
    WHERE table_schema = 'public' AND table_name = 'quiz_questions' AND column_name = 'reading_text_id'
  ) THEN
    ALTER TABLE public.quiz_questions ADD COLUMN reading_text_id UUID REFERENCES public.quiz_reading_texts(id) ON DELETE SET NULL;
    RAISE NOTICE 'Coluna reading_text_id adicionada em quiz_questions';
  ELSE
    RAISE NOTICE 'Coluna reading_text_id já existe em quiz_questions';
  END IF;
END $$;
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns
    WHERE table_schema = 'public' AND table_name = 'quiz_questions' AND column_name = 'display_order'
  ) THEN
    ALTER TABLE public.quiz_questions ADD COLUMN display_order INTEGER DEFAULT 0;
    RAISE NOTICE 'Coluna display_order adicionada em quiz_questions';
  ELSE
    RAISE NOTICE 'Coluna display_order já existe em quiz_questions';
  END IF;
END $$;
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns
    WHERE table_schema = 'public' AND table_name = 'quiz_questions' AND column_name = 'question_number'
  ) THEN
    ALTER TABLE public.quiz_questions ADD COLUMN question_number VARCHAR(10);
    RAISE NOTICE 'Coluna question_number adicionada em quiz_questions';
  ELSE
    RAISE NOTICE 'Coluna question_number já existe em quiz_questions';
  END IF;
END $$;

CREATE INDEX IF NOT EXISTS idx_quiz_questions_reading_text ON public.quiz_questions(reading_text_id);

-- BLOCO 17: Criar tabela QUIZ_ATTEMPTS
...
-- BLOCO 18: Criar tabela QUIZ_ANSWERS
...
-- BLOCO 19: Criar tabela QUIZ_QUESTION_STATS
...
-- BLOCO 20: Função CAN_USER_ACCESS_QUIZ
...
-- BLOCO 21: Função CALCULATE_QUIZ_RESULT
...
-- BLOCO 22: Função SUBMIT_QUIZ_ATTEMPT
...
-- BLOCO 23: Função VALIDATE_ANSWER_SHEET
...
-- BLOCO 24: Trigger para UPDATE_UPDATED_AT
...
-- BLOCO 26: Policies QUIZ_CLASSES
...
-- BLOCO 27: Policies QUIZ_READING_TEXTS
...
-- BLOCO 28: Policies QUIZ_ATTEMPTS
...
-- BLOCO 29: Policies QUIZ_ANSWERS
...
-- BLOCO 30: Policies QUIZ_QUESTION_STATS
