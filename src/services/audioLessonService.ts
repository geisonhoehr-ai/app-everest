...
export const audioLessonService = {
  ...
  async getAudioLessons(): Promise<AudioLesson[]> {
    try {
      console.log('🔍 Fetching audio lessons from database...')
      ...
      console.log('✅ Found audio lessons:', lessons?.length || 0)
      ...
      console.error('Erro ao buscar audioaulas:', error)
      return []
    }
  },
  ...
  async getAudioLessonById(id: string): Promise<AudioLesson | null> {
    try {
      ...
      console.error('Erro ao buscar audioaula:', error)
      return null
    }
  },
  ...
  async incrementListens(id: string): Promise<void> {
    try {
      ...
      console.error('Erro ao incrementar contador de ouvintes:', error)
    }
  }
}
